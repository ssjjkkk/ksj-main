...
public class ReplyController {
	...
	public ResponseEntity<String> remove(@PathVariable int bno, @PathVariable int rno, @PathVariable int page){
		ResponseEntity<String> entity = null;
		...
		try {
			...
			Map<String, Object> dataMap = replyService.selectReplyList(bno, cri);
			PageMaker pageMaker = (PageMaker)dataMap.get("pageMaker");
			int startPage = pageMaker.getCri().getPage();
			ArrayList<ReplyVO> replyList =(ArrayList<ReplyVO>) dataMap.get("replyList");
			if(replyList.isEmpty()) {
				startPage = pageMaker.getCri().getPage() - 1;
			}
